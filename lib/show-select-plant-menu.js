...
export const showSelectPlantMenu = () => {
  ...
  values.vueApp.plantMenuOpen = true
  ...
  values.addPlantMenuConfirmButton = createSprite('button_confirm', async () => {
    ...
    if (added) {
      ...
      if (!values.vueApp.stats.planted[plant.key]) values.vueApp.stats.planted[plant.key] = 0
      values.vueApp.stats.planted[plant.key] += 1
      apply()
      cloudSave()
